    ...
    IRandomGenerator randomGenerator)
{
    ...
    public async Task<IReadOnlyList<PersonResult>> SearchPeople(
        ...
        [Description("Optional role filter. Candidates with no credit in this role are excluded, and the returned CreditCounts and SampleWorks are narrowed to it. Valid values come from get_roles. Call get_roles if unsure. Do not invent other values.")] string role = null,
    ...
    [McpServerTool(ReadOnly = true), Description("Search playable tracks by any combination of title, person/group, role and album. Each result carries its album, year, length and full credits, so no extra lookup is needed to describe it. At least one of titleQuery, personId or albumId is required. Title matching ranks a capped set of candidates, so a title made of common words can crowd out the right track: resolve the person with search_people or the album with search_albums first and pass personId or albumId whenever you can, and narrow further if the result comes back with Truncated true. Outcome: Matched means at least one candidate reached minScore and is returned; WeakMatch means candidates exist but none reached minScore, so nothing is returned and TopScore plus Hint say how close the best one came; NoMatch means nothing exists in this scope. Read Hint for the next step.")]
    [RequirePermission(CorePermissions.AlbumsView, CorePermissions.AlbumsViewAll)]
    public async Task<TrackSearchResult> SearchTracks(
        ...
        [Description("Optional person/group ID to filter by, as returned by search_people. Use 0 or omit to not filter by person.")] int personId = 0,
        [Description("Optional role filter. Valid values come from get_roles. Call get_roles if unsure. Do not invent other values. Only meaningful together with personId.")] string role = null,
        [Description("Optional album ID to filter by, as returned by search_albums. Use 0 or omit to not filter by album.")] int albumId = 0,
        ...
        [Description("Minimum fuzzy-match score in the range 0.0-1.0. Candidates below it are excluded, so raising it narrows the results and lowering it widens them. Leave at the default for loose phrasing; raise it to about 0.7 when you believe you have the exact title. Do not go below 0.3, where matches stop being meaningful.")] double minScore = 0.4)
    {
        ...
        {
            throw new McpException("The 'role' filter only works together with 'personId'. To filter by a named artist, first call search_people to resolve them to a personId, then pass that personId here.");
        }
    ...
    [McpServerTool(ReadOnly = true), Description("Full disc/track tree for one album, in order, including unplayable tracks (IsPlayable false), for verifying an externally-suggested title against what actually exists and for queueing a complete album. Never pass a track with IsPlayable false to add_to_queue.")]
    [RequirePermission(CorePermissions.AlbumsView, CorePermissions.AlbumsViewAll)]
    public async Task<AlbumTracklist> GetAlbumTracklist(
        ClaimsPrincipal user,
        [Description("The album ID whose full disc/track tree to retrieve, as returned by search_albums or search_tracks.")] int albumId)
    {
        bool allowOnlyPublicAlbums = !await permissionAuthorizer.HasAnyAsync(user, CorePermissions.AlbumsViewAll);
    ...
    public async Task<TrackPickResult> PickTracks(
        ClaimsPrincipal user,
        [Description("The person/group ID to pick tracks for, as returned by search_people. Must be a positive ID.")] int personId,
        [Description("Optional role filter. Valid values come from get_roles. Call get_roles if unsure. Do not invent other values.")] string role = null,
        [Description("Number of tracks to pick (1-50). Fewer may come back if the eligible pool is smaller.")] int count = 1)
    {
        if (personId <= 0)
        {
            throw new McpException("personId must be a positive ID, as returned by search_people.");
        }
    ...
    public async Task<QueueAddResult> AddToQueue(
        ClaimsPrincipal user,
        [Description("Array of integer track IDs to enqueue, e.g. [123, 456]. Obtain these from search_tracks, pick_tracks or get_album_tracklist.")] List<int> trackIds,
