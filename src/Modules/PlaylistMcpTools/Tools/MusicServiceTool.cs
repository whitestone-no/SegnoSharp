    ...
    IRandomGenerator randomGenerator)
{
    ...
    [McpServerTool(ReadOnly = true), Description("Resolve albums by title, by credited person, or both; at least one of query and personId is required. Returns album-level credits, a match score, and totalMatches: the number of albums that matched before the limit was applied. When totalMatches is larger than the list you got back, you are holding a slice, not the whole set, so raise limit or say so. If truncated is true, a title search matched more albums than the ranking looks at, so the results came from a subset: search a more specific title or add a personId rather than trusting the order. Use personId on its own to answer 'what else do we have by X'; do not group the results of a track search to answer that, because a track search stops at its own limit and will under-report. Prefer the plainest exact title match: sequels ('... II') and qualified editions ('... (Complete Rejected Score)', deluxe, live) should only be chosen when the user asked for them.")]
    [RequirePermission(CorePermissions.AlbumsView, CorePermissions.AlbumsViewAll)]
    ...
    public async Task<TrackSearchResult> SearchTracks(
        ...
        [Description("Maximum number of tracks to return (1-100). Compare it against totalMatches before calling a list complete.")] int limit = 10,
    ...
    public async Task<QueueAddResult> AddToQueue(
        ...
        [Description("Set true only after the user has been told how many tracks this will add and has answered that they want it. Their answer, not your assumption. Required above 25 tracks, which the tool refuses without it; leave false otherwise.")] bool confirmed = false)
    {
